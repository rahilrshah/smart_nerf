...
def run_command(command, cwd="."):
    ...
    try:
        # We only call Python scripts now, which is much more stable
        subprocess.run(command, check=True, cwd=cwd)
    except subprocess.CalledProcessError as e:
        print(f"\n--- ERROR ---"); print(f"Command failed with exit code {e.returncode}"); exit()
    except FileNotFoundError:
        print(f"\n--- ERROR ---"); print(f"Command not found: '{command[0]}'."); exit()

def pre_flight_checks():
    """Check if necessary script files exist."""
    print("--- Running Pre-flight Checks ---")
    if not os.path.exists("vendor/instant-ngp/scripts/run.py"):
        print("❌ ERROR: Official training script not found at 'vendor/instant-ngp/scripts/run.py'."); exit()
...
def main(args):
    ...
    if not os.path.exists(dataset_source_path): print(f"❌ ERROR: Dataset source directory not found at '{dataset_source_path}'."); exit()
    ...
    if args.mode in ['full', 'optimize_only']:
        ...
        for i in range(1, args.max_iterations + 1):
            ...
            # Step 2a: Train scout model using the OFFICIAL script with proven flags.
            ...
            # Step 2b: Analyze the generated snapshot with our analysis script
            ...
            if not os.path.exists(recommendations_file) or os.path.getsize(recommendations_file) == 0:
                print("\n✅ Analysis complete. No new images recommended."); break
            if i == args.max_iterations: print("\nReached max iterations."); break
    ...
    # --- Part 2: Final Meshing with Neuralangelo (Restored) ---
